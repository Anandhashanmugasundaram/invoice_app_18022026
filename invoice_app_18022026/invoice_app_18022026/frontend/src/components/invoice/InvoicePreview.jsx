...
const InvoicePreview = ({ invoiceState, onStateUpdate }) => {
  ...
  const refreshHtmlPreview = useCallback(async (state) => {
    ...
    try {
      const html = await billInvoiceApi.getHtmlPreview(state);
      setHtmlContent(html);
      setError(null);
      return true;
    } catch (err) {
  ...
  const handleDeleteRow = useCallback(async (rowIndex) => {
    ...
    try {
      setDeletingRow(true);
      isInternalUpdateRef.current = true;
      const response = await billInvoiceApi.deleteRow(stateRef.current, rowIndex);
      if (response && response.state) {
        stateRef.current = response.state;
  ...
  const handleAddRow = async () => {
    ...
    try {
      setAddingRow(true);
      const response = await billInvoiceApi.addRow(stateRef.current);
      stateRef.current = response.state;
      onStateUpdate(response.state);
      setLastSaved(new Date());
    } catch (err) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200">
      ...
      {/* <div>
          <p>you can also edit the client details in the form above</p>
        </div> */}
